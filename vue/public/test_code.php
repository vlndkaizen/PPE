<div class="quiz-container" style="max-width: 900px; margin: 0 auto;">
    ...
    <form method="post" action="index.php?page=4">
        ...
        foreach($questions as $i => $q) {
            ...
            echo '<div class="question-number">Question ' . $num . ' / 20</div>';
            echo '<div class="question-text">' . htmlspecialchars($q['q']) . '</div>';
            
            foreach($q['a'] as $j => $option) {
    ...
    if(isset($_POST['submit_quiz'])) {
        ...
        echo '<div class="score-display" style="background: ' . $color . ';">';
        echo '<h2>Résultat de votre examen</h2>';
        echo '<div class="score-number">' . $score . ' / ' . $total . '</div>';
        echo '<p style="font-size: 1.5rem;">Note : ' . $note . ' / 40</p>';
        echo '<p style="margin-top: 20px; font-size: 1.2rem;">' . $appreciation . '</p>';
        echo '<a href="index.php?page=4" class="btn" style="margin-top: 30px; background: white; color: ' . $color . '; display: inline-block; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600;">Recommencer</a>';
        echo '</div>';
    }
    ?>
</div>